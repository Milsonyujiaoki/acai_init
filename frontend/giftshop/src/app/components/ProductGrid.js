...
export function ProductGrid({ onAddToCart, selectedCategory, searchQuery }) {
    ...
    return (_jsx("section", { className: "py-12", children: _jsxs("div", { className: "container mx-auto px-4", children: [_jsxs("div", { className: "flex items-center justify-between mb-8", children: [_jsx("h2", { children: selectedCategory === 'all'
                                ? 'All Gifts'
                                : `${selectedCategory.charAt(0).toUpperCase() + selectedCategory.slice(1)} Gifts` }), _jsxs("p", { className: "text-muted-foreground", children: [filteredProducts.length, " product", filteredProducts.length !== 1 ? 's' : ''] })] }), filteredProducts.length === 0 ? (_jsx("div", { className: "text-center py-12", children: _jsx("p", { className: "text-muted-foreground", children: "No products found matching your criteria." }) })) : (_jsx("div", { className: "grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-6", children: filteredProducts.map((product) => (_jsx(ProductCard, { product: product, onAddToCart: onAddToCart }, product.id))) }))] }) }));
}
