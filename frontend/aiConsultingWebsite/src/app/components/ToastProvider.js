...
import { CheckCircle, AlertCircle, Info, X, AlertTriangle, Zap, Crown, Gift, Bell, Star, Heart, Lightbulb } from "lucide-react";
...
export function ToastProvider({ children }) {
    ...
    return (_jsxs(ToastContext.Provider, { value: value, children: [children, Object.entries(toastsByPosition).map(([position, positionToasts]) => (_jsx("div", { className: `fixed z-50 pointer-events-none ${getPositionClasses(position)}`, style: { maxWidth: '420px' }, children: _jsx("div", { className: "space-y-3", children: _jsx(AnimatePresence, { children: positionToasts.map((toast) => {
                            const styles = getToastStyles(toast.type);
                            const IconComponent = styles.icon;
                            return (_jsx(motion.div, { layout: true, initial: {
                                    opacity: 0,
                                    scale: 0.8,
                      ...
                      glass-heavy
                    `, children: _jsxs("div", { className: "p-4", children: [_jsxs("div", { className: "flex items-start gap-3", children: [_jsx("div", { className: "flex-shrink-0 mt-0.5", children: toast.type === 'loading' ? (_jsx(motion.div, { animate: { rotate: 360 }, transition: { duration: 2, repeat: Infinity, ease: "linear" }, children: _jsx(IconComponent, { className: `w-5 h-5 ${styles.iconColor}` }) })) : (_jsx(IconComponent, { className: `w-5 h-5 ${styles.iconColor}` })) }), _jsxs("div", { className: "flex-1 min-w-0", children: [toast.title && (_jsx("h4", { className: `text-sm font-semibold ${styles.textColor} mb-1`, children: toast.title })), _jsx("p", { className: `text-sm ${styles.textColor} leading-relaxed`, children: toast.message }), toast.action && (_jsx("div", { className: "mt-3", children: _jsx("button", { onClick: toast.action.onClick, className: `text-sm font-medium ${styles.iconColor} hover:underline focus:outline-none focus:underline`, children: toast.action.label }) }))] }), _jsx("button", { onClick: () => removeToast(toast.id), className: `flex-shrink-0 ${styles.iconColor} hover:opacity-70 transition-opacity focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-blue-500 rounded-md p-1`, children: _jsx(X, { className: "w-4 h-4" }) })] }), !toast.persistent && toast.type !== 'loading' && (_jsx(motion.div, { className: `mt-3 h-1 ${styles.iconColor.replace('text-', 'bg-')} opacity-30 rounded-full overflow-hidden`, children: _jsx(motion.div, { className: `h-full ${styles.iconColor.replace('text-', 'bg-')} rounded-full`, initial: { width: '100%' }, animate: { width: '0%' }, transition: {
...
export const commonToasts = {
    ...
    // E-commerce
    addedToCart: (item) => toast.success(`${item} added to cart.`),
    removedFromCart: (item) => toast.info(`${item} removed from cart.`),
    ...
    // File Operations
    fileUploaded: (filename) => toast.success(`${filename} uploaded successfully.`),
    fileUploadError: () => toast.error("File upload failed. Please try again."),
    fileDownloaded: (filename) => toast.success(`${filename} downloaded.`),
    // Form Submissions
    ...
    // Feature Updates
    newFeature: (feature) => toast.info(`New feature available: ${feature}`, {
